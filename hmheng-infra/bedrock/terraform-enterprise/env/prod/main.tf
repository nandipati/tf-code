module "brcoredev-infra" {
  ...
  elb_subnet_id      = "${element(split(",", data.terraform_remote_state.infra.subnet_core-haproxy_nlb_ext_list), 0)}"
  ...
  zone_id            = "${data.terraform_remote_state.infra.hmhco_eng_public_zone_id}"
  hostname           = "terraform"
  internal_elb       = "false"
