...
resource "aws_security_group" "cds-content-provider-cache-sg" {
    vpc_id = "${var.vpc_id}"
...
resource "aws_elasticache_cluster" "cds-content-provider-elasticache" {
    ...
    security_group_ids = [
      "${aws_security_group.cds-content-provider-cache-sg.id}"
    ]
...
resource "aws_route53_record" "cds-content-provider-elasticache" {
  zone_id = "Z2WUOZOGBH83LP"
  name = "cds-content-provider-${var.tag_stage}-elasticache"
  type = "CNAME"
  ttl = "60"
  records = ["${ aws_elasticache_cluster.cds-content-provider-elasticache.cache_nodes.0.address }"]
}
