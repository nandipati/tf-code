resource "aws_security_group" "hmheng-qe-pactbroker-db-sg" {
  vpc_id = "${var.vpc_id}"
...
resource "aws_db_parameter_group" "hmheng-qe-pactbroker-params" {
  name   = "${var.db_identifier}-${var.stage}-params"
  family = "${var.db_parameter_group_family}"
}

resource "aws_db_instance" "hmheng-qe-pactbroker-db" {
  ...
  parameter_group_name        = "${aws_db_parameter_group.hmheng-qe-pactbroker-params.id}"
  allow_major_version_upgrade = true
  auto_minor_version_upgrade  = true

  vpc_security_group_ids = [
    "${aws_security_group.hmheng-qe-pactbroker-db-sg.id}",
  ]
