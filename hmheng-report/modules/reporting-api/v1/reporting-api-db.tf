resource "aws_security_group" "hmheng-report-reporting-api-db-sg" {
  vpc_id = "${var.vpc_id}"
...
resource "aws_db_parameter_group" "hmheng-report-reporting-api-new-wal-params" {
...
resource "aws_db_instance" "hmheng-report-reporting-db" {
  ...
  parameter_group_name        = "${aws_db_parameter_group.hmheng-report-reporting-api-new-wal-params.id}"
  ...
  vpc_security_group_ids = [
    "${aws_security_group.hmheng-report-reporting-api-db-sg.id}",
  ]
...
resource "aws_db_instance" "hmheng-report-reporting-db-rr01" {
  allocated_storage           = "${var.db_allocated_storage}"
  identifier                  = "${var.db_identifier}-rr-${var.stage}"
  replicate_source_db         = "${aws_db_instance.hmheng-report-reporting-db.identifier}"
  engine                      = "${var.db_engine}"
  engine_version              = "${var.db_engine_version}"
  instance_class              = "${var.db_class}"
  name                        = "${var.db_name}"
  ...
  tags {
    Project     = "br_${var.db_identifier}-rr_${var.stage}_db"
    environment = "${var.tag_environment}"
    stage       = "${var.stage}"
    cost        = "${var.tag_cost}"
  }
