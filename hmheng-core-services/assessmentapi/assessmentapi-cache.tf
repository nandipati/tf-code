...
resource "aws_security_group" "assessmentapi-cache-sg" {
  vpc_id = "${var.vpc_id}"
...
resource "aws_elasticache_cluster" "assessmentapi-elasticache" {
  ...
  security_group_ids = [
    "${aws_security_group.assessmentapi-cache-sg.id}"
  ]
...
resource "aws_route53_record" "assessmentapi-elasticache" {
  zone_id = "Z2WUOZOGBH83LP"
  name = "assessmentapi-${var.tag_stage}-elasticache"
  type = "CNAME"
  ttl = "60"
  records = ["${ aws_elasticache_cluster.assessmentapi-elasticache.cache_nodes.0.address }"]
}
