...
resource "aws_security_group" "content-catalog-cache-sg" {
  vpc_id = "${var.vpc_id}"
...
resource "aws_elasticache_cluster" "content-catalog-elasticache" {
  ...
  security_group_ids = [
    "${aws_security_group.content-catalog-cache-sg.id}",
  ]
...
resource "aws_route53_record" "content-catalog-elasticache" {
  zone_id = "${var.zone_id}"
  name    = "content-catalog-elasticache${var.tag_stage == "prod" ? "" : ".${var.tag_stage}"}"
  type    = "CNAME"
  ttl     = "60"
  records = ["${ aws_elasticache_cluster.content-catalog-elasticache.cache_nodes.0.address }"]
}
