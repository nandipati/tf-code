...
resource "aws_security_group" "clm-entitlements-cache-sg" {
    vpc_id = "${var.vpc_id}"
...
resource "aws_elasticache_cluster" "clm-entitlements-elasticache" {
    ...
    security_group_ids = [
      "${aws_security_group.clm-entitlements-cache-sg.id}"
    ]
...
resource "aws_route53_record" "clm-entitlements-elasticache" {
  zone_id = "Z2WUOZOGBH83LP"
  name = "clm-entitlements-${var.tag_stage}-elasticache"
  type = "CNAME"
  ttl = "300"
  records = ["${ aws_elasticache_cluster.clm-entitlements-elasticache.cache_nodes.0.address }"]
}