resource "aws_security_group" "hmheng-reading-inventory-db-sg" {
  vpc_id = "${var.vpc_id}"
...
resource "aws_security_group_rule" "ingress_cidr" {
  ...
  security_group_id = "${aws_security_group.hmheng-reading-inventory-db-sg.id}"
}
...
resource "aws_rds_cluster_instance" "cluster_instances" {
  ...
  cluster_identifier = "${aws_rds_cluster.reading-inventory-api.id}"
...
resource "aws_rds_cluster" "reading-inventory-api" {
  ...
  vpc_security_group_ids = [
    "${aws_security_group.hmheng-reading-inventory-db-sg.id}",
  ]
}
